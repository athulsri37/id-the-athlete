-- Tennis (women's) sport row and its attribute definitions.
...
--
-- Sibling to Tennis/Men/00-sport-and-attributes.sql: a separate Sports row
-- (its own Id), scoped to its own AttributeDefinitions via WHERE
-- s."Slug" = 'tennis-women' below -- never shared with or duplicated from
-- Men's Tennis. Player data is deliberately not seeded yet; player-batch
-- files land in a follow-up once the roster is curated and verified.

INSERT INTO "Sports" ("Name", "Slug")
VALUES ('Women''s Tennis', 'tennis-women')
