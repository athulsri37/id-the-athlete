-- Women's International Cricket sport row and its attribute definitions.
...
--
-- Sibling to Cricket/MenInternational/00-sport-and-attributes.sql: a
-- separate Sports row (its own Id), scoped to its own AttributeDefinitions
-- via WHERE s."Slug" = 'cricket-women-international' below -- never shared
-- with or duplicated from Men's International Cricket, the same isolation
-- pattern already used for Men's/Women's Tennis. Player data is
...
VALUES ('Women''s International Cricket', 'cricket-women-international')
...
WHERE ad."SportId" = s."Id"
  AND s."Slug" = 'cricket-women-international'
